...
class SubmissionV2DataSheet implements FromArray, WithTitle, WithStyles
{
    /** Tracks row numbers that need distinct styling */
    ...
    private function sectionTitle(array &$rows, string $code, string $title): void
    {
        $rows[] = [];
        $rows[] = ["{$code} - {$title}"];
        $this->sectionTitleRows[] = count($rows); // 1-based
    }
    ...
    public function array(): array
    {
        ...
        $rows[] = ['DATA INSTRUMEN PENJAMINAN MUTU SMK'];
        $this->sectionTitleRows[] = 1;
        ...
        // Build flat key→value map from rows
        $rowsMap = [];
    ...
    public function styles(Worksheet $sheet): void
    {
        ...
        foreach ($this->sectionTitleRows as $rowNum) {
            $sheet->getStyle("A{$rowNum}:" . $sheet->getHighestColumn() . "{$rowNum}")->applyFromArray($styleSection);
        }
        foreach ($this->columnHeaderRows as $rowNum) {
            $sheet->getStyle("A{$rowNum}:" . $sheet->getHighestColumn() . "{$rowNum}")->applyFromArray($styleColHeader);
        }
        foreach ($this->subTitleRows as $rowNum) {
            $sheet->getStyle("A{$rowNum}:" . $sheet->getHighestColumn() . "{$rowNum}")->applyFromArray($styleSubTitle);
        }
        ...
        // Set a reasonable max width for wider columns
