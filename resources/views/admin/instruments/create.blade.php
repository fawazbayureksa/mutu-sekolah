...
@section('title', $instrument ? 'Edit Instrument' : 'Create New Instrument')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-0">{{ $instrument ? 'Edit Instrument' : 'Create New Instrument' }}</h2>
        <p class="text-muted mb-0">{{ $instrument ? 'Update instrument details' : 'Create a new assessment instrument' }}</p>
    </div>
...
<div class="row">
    <div class="col-lg-8">
        <div class="card">
            <div class="card-body">
                <form action="{{ $instrument ? route('admin.instruments.update', $instrument) : route('admin.instruments.store') }}" method="POST">
                    @if($instrument)
                        @method('PUT')
                    @endif
                    @csrf
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="code" class="form-control" value="{{ old('code', $instrument->code ?? '') }}" required>
                            @error('code')
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <select name="category" class="form-select">
                                ...
                                <option value="akreditasi" {{ old('category', $instrument->category ?? '') === 'akreditasi' ? 'selected' : '' }}>Akreditasi</option>
                                <option value="sertifikasi" {{ old('category', $instrument->category ?? '') === 'sertifikasi' ? 'selected' : '' }}>Sertifikasi</option>
                                <option value="pelatihan" {{ old('category', $instrument->category ?? '') === 'pelatihan' ? 'selected' : '' }}">Pelatihan</option>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label">Version</label>
                            <input type="text" name="version" class="form-control" value="{{ old('version', $instrument->version ?? '1.0') }}">
                        </div>
                    </div>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" name="estimated_duration" class="form-control" value="{{ old('estimated_duration', $instrument->estimated_duration ?? '') }}">
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="card bg-light p-3 mb-4">
                        ...
                        <table class="table table-sm">
                            ...
                            <tbody>
                                @foreach($questions as $question)
                                    <tr>
                                        <td>
                                            <input type="checkbox" class="form-check-input question-checkbox" name="questions[]" value="{{ $question->id }}">
                                        </td>
                                        <td>
                                            <strong>{{ $question->question_code ?? 'Q' . $loop->iteration }}</strong>
                                            <small class="text-muted">{{ Str::limit($question->question_text, 80) }}</small>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            @if($question->indicator && $question->indicator->aspect)
                                                <span class="badge bg-info">{{ $question->indicator->aspect->name }}</span>
                                            @else
                        ...
                        <div class="mt-3">
                            <button type="button" class="btn btn-outline-secondary btn-sm" onclick="openQuestionSelector()">
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-lg me-2"></i>{{ $instrument ? 'Update Instrument' : 'Create Instrument' }}
                        </button>
