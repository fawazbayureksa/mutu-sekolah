...
@section('title', $user ? 'Edit User' : 'Create New User')

@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-0">{{ $user ? 'Edit User' : 'Create New User' }}</h2>
        <p class="text-muted mb-0">{{ $user ? 'Update user information' : 'Add a new user to the system' }}</p>
    </div>
                ...
                <form action="{{ $user ? route('admin.users.update', $user) : route('admin.users.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        <div class="form-check">
                            <input type="checkbox" name="is_active" id="is_active" class="form-check-input"
                                   {{ $user && $user->is_active ? 'checked' : old('is_active', true) ? 'checked' : '' }}>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-lg me-2"></i>{{ $user ? 'Update User' : 'Create User' }}
                        </button>
    ...
    <div class="col-lg-4">
        ...
        @if($user)
            <div class="card">
                ...
                <div class="card-body">
                    <p class="mb-1"><strong>Created:</strong> {{ $user->created_at->format('F j, Y') }}</p>
                    <p class="mb-1"><strong>Last Login:</strong> {{ $user->last_login_at ? $user->last_login_at->diffForHumans() : 'Never' }}</p>
