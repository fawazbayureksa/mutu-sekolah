...
@section('content')
    <div class="container-fluid">
        ...
        {{-- ② Review Jawaban --}}
        <form action="{{ route('verifier.submissions-v2.section-notes', $submission) }}" method="POST">
            @csrf
                    ...
                    @if (in_array($submission->status, ['submitted', 'verified']))
                        <div class="d-flex gap-2 align-items-center">
                            <button type="button" class="btn btn-outline-secondary btn-sm" onclick="toggleReviewForm()">
                                <i class="bi bi-pencil-square me-1"></i> <span id="toggleBtnText">Buka Form Feedback</span>
                            </button>
                            <div class="submit-feedback-bar d-none">
                                <button type="submit"
                                    formaction="{{ route('verifier.submissions-v2.section-notes', $submission) }}"
                    ...
                    <div class="card-footer bg-white border-top py-3 submit-feedback-bar d-none">
                        <div class="mb-3">
                            <label class="form-label fw-semibold small text-dark"><i
                                    class="bi bi-chat-left-text me-1"></i>Catatan Verifikasi Keseluruhan (Opsional)</label>
                            <textarea name="notes" class="form-control form-control-sm" rows="2"
                                placeholder="Masukkan catatan umum untuk pengajuan ini..."></textarea>
                        </div>
                        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
                            <span class="text-muted small"><i class="bi bi-info-circle me-1"></i>Pilih aksi untuk
                                menyimpan catatan per bagian atau menyelesaikan verifikasi.</span>
                            <div class="d-flex gap-2">
                                <button type="submit"
                                    formaction="{{ route('verifier.submissions-v2.section-notes', $submission) }}"
                                    class="btn btn-outline-primary btn-sm">
                                ...
                                @if ($submission->status === 'submitted')
                                    <button type="submit"
                                        formaction="{{ route('verifier.submissions-v2.verify', $submission) }}"
